...
- **Minimum funding to create a commitment:** arming a commitment requires available balance ≥ **Rs. 10** (configurable); **max penalty/forfeit exposure capped to the pre-funded/staked balance at creation** — no money, no commitment. Underfunded → `402 COMMITMENT_FUNDING_REQUIRED`. See [product/prd.md](product/prd.md) FR-6.
- **Funds exhausted mid-commitment:** if the stake is fully forfeited, **enforcement (blocking/limits) continues for free** until the period ends; the money layer goes dormant (no further penalties; unlocks/break-fees unavailable; new commitments blocked until top-up) — prevents "burn the stake to escape." See [product/prd.md](product/prd.md) FR-6.
...
- **Top-up fees:** **transparent gross-up** — user picks a wallet credit amount; charged amount + disclosed processing fee; wallet credited the round amount; fee-neutral (no silent net shortfall). **Min top-up Rs. 20**; wallet-balance cap by KYC tier; withdrawals bear their own fee + cycle limit. See [payments/payment-architecture.md](payments/payment-architecture.md).
