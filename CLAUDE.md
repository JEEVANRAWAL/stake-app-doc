...
- **Minimum funding to create a commitment:** arming a commitment (staked deposit **or** penalty-bearing restrictions) requires available balance ≥ **Rs. 200** (configurable); **max penalty/forfeit exposure capped to the pre-funded/staked balance at creation** — no money, no commitment. Underfunded → `402 COMMITMENT_FUNDING_REQUIRED`. See `docs/product/prd.md` FR-6.
