...
- **Top-up fees:** **transparent gross-up** (wallet credit + disclosed processing fee; fee-neutral; no silent net shortfall). Min top-up Rs. 20; wallet cap by KYC tier; withdrawals bear own fee + cycle limit. See `docs/payments/payment-architecture.md`.
...
- **Minimum funding to create a commitment:** arming a commitment (staked deposit **or** penalty-bearing restrictions) requires available balance ≥ **Rs. 10** (configurable); **max penalty/forfeit exposure capped to the pre-funded/staked balance at creation** — no money, no commitment. Underfunded → `402 COMMITMENT_FUNDING_REQUIRED`. See `docs/product/prd.md` FR-6.
